...
function SoapClient(clientOptions, soapCallback) {
    if (typeof(soapCallback) != 'function')
            ...
            var request = http.request(options, function(response) {
                if (response.statusCode >= 300 && response.statusCode < 400) {
                    if (currentRedirects < MAX_REDIRECTS) {
                        ...
                        options.url = url.resolve(options.url, response.headers.location);

                        beginRead(options);
                        return;
                    }
                    else {
                        options.callback(new Error('Too many redirects: ' + currentRedirects), null);
                        return;
                    }
                }
                ...
                response.on('end', function () {
                    ...
                    new DomJS().parse(body, function(err, responseDom) {
                        options.callback(err, responseDom);
                    });
                });
            });
    ...
    function checkAuthorization(httpOptions) {
    ...
    function invokeMethod(methodName, arguments, inputType, outputType, endpoint) {
        ...
        readHttp({
            ...
            method: 'POST',
            callback: function(err, responseDom) {
                if (err) {
                    callback(err, null);
                    return;
                }
